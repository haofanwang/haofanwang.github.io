...
Previously, we made substantial breakthroughs around tasks like "stylization," "identity preservation," and "text rendering." The next step is to discover and define new tasks. This is by no means easy, as it requires keen insight into users' deeper needs and pain points. We're shifting from technology-centric development to user-driven solution design. Don't be limited by old tasks. Any pain point a user experiences during use can potentially become a new task. I'd like to share a few examples.

The shift from CLIP-conditioned short prompts to VLM-conditioned long prompts understanding has undoubtedly enhanced the macro coherence of generated images. However, it has also exposed a fundamental gap: reliably translating specific, micro-level phrases within a complex prompt into precise visual elements continues to be a major hurdle. FIBO[^27] addresses this limitation by training on long structured (JSON format) captions, where every training sample is annotated with the same set of fine-grained attributes. This design enables disentangled control over visual factors like lighting, composition, and color. Another pertinent example can be found in the increasingly popular domain of instruction-based image editing. While current models allow users to readily modify image through natural language instructions, the nuanced control over the intensity of these edits has remained a significant and under-addressed challenge. Addressing this gap, Kontinuous Kontext[^28] introduces a novel dimension of control by enabling continuous adjustment of edit strength. This approach allows users to seamlessly transition from the original image to a fully modified result through fine-grained, continuous interpolation, rather than being constrained to a binary or coarse-grained choice. 

From an implementation perspective, these works themselves might not be overly complex, but by defining new input, they open up new avenues of thought. Let me venture a bolder prediction: We are already extensively utilizing interleaved data in training, and the next logical evolution may involve generating not just visual content, but jointly predicting interleaved image-text outputs. New tasks can not only arise from new inputs, but may also be closely related to the output form.

### Model as Product: The Shift from Researcher to Product Manager
For AI application companies, whether a model is open-source will become less critical. The real value lies in: how target users can use it in specific scenarios. This requires more context from the product side. In the near future, the line between researcher and product manager will blur, with research directions and product requirements becoming more tightly integrated. It is increasingly essential for researchers to cultivate a product-centric mindset.

The visual generative products that impressed me most this year are Reve and Sora2. Reve[^29] novelly introduced regional control in image editing, allowing users to seamlessly manipulate object positions using bounding boxes. When a user uploads an image, the understanding model identifies the various parts of the image and assigns them corresponding bounding boxes. Users can then control the spatial arrangement of these elements by moving and modifying the position and size of the bboxes. This interactive design is refreshing. Sora2 a similar story, it is no longer a standalone video generation model, but is closely integrated with the Sora app and its core Cameo[^30] capabilities. Multi-shot scenes, audio-visual synchronization, and cross-shot character consistency – these capabilities are deeply intertwined with the product form. The capabilities of the model are no longer standalone, but closely related to product design.

In Lovart, our team's recent exploration in Layered Image Editing[^31] follows the same logic. At the beginning, our product team found that in design scenarios, users accustomed to Photoshop-like tools are used to the concept of layers instead of instructed editing, where each individual slice can be tacked or moved to create their image composition. However, layers have not received widespread attention in our field. Therefore, starting from the real habits of users in design scenarios, we envisioned converting elements in images into individual image and text layers, just like PSD templates in Canva, which sounds like a cool thing, right? And then we looked for specific technical solutions. This might become the future research paradigm: before training even begins, collaborate with product teams to plan which features are worth internalizing, consciously collect data for them, and build differentiated capabilities into commercial "silver bullets." Creativity is paramount.

### Advancements in Models Will Create Unforeseen Consumer Value
As we have repeatedly emphasized, model performance continues to improve steadily. Even tasks with currently unsatisfactory results will likely be resolved in the near future. In stark contrast, however, the field of visual generation—particularly image generation—has yet to deliver commensurate consumer value. In the earliest days of image generation, aside from community enthusiasts, it was difficult to imagine anyone willingly sharing AI-generated images on social media every day. Yet, as image quality improved and personalized generation matured, people gradually began using AI images in limited scenarios—such as for ID photos. Now, with progress in image editing, we can inject imaginative elements into otherwise mundane photos, creating playful and engaging outcomes. This trajectory demonstrates that model advancements can indeed generate consumer value that previously did not exist. A notable example is Sora2’s Cameo, which introduces a social dimension by defining specific usage contexts for characters——a promising step in this direction. So, stay optimistic.
...
[^24]: Rinon Gal, et al. [An Image is Worth One Word: Personalizing Text-to-Image Generation using Textual Inversion](https://arxiv.org/abs/2208.01618)
[^25]: Llion Jones. [https://tedai-sanfrancisco.ted.com/speakers/2025/llion-jones/](https://tedai-sanfrancisco.ted.com/speakers/2025/llion-jones/)
[^26]: Jared Kaplan, et al. [Scaling Laws for Neural Language Models](https://arxiv.org/abs/2001.08361)
[^27]: Eyal Gutflaish, et al. [Generating an Image From 1,000 Words: Enhancing Text-to-Image With Structured Captions](https://arxiv.org/abs/2511.06876)
[^28]: Rishubh Parihar, et al. [Kontinuous Kontext: Continuous Strength Control for Instruction-based Image Editing](https://arxiv.org/abs/2510.08532)
